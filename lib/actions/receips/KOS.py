...
def build(game_dir, project_dir):
    src_dir = SourceDir(game_dir, project_dir.joinpath("src"))
    ...
    kos_safe_src.std_compile(references=[bin_target.joinpath("ICSharpCode.SharpZipLib.dll")])
    ...
    kos_src.std_compile(references=["Assembly-CSharp.dll", "Assembly-CSharp-firstpass.dll", "UnityEngine.dll", "UnityEngine.UI.dll", kos_safe_src.output])

    logging.info("  Build kOS.Safe.Test")
    test_dependencies = [
            kos_safe_src.output,
            src_dir.joinpath("packages", "NUnit.2.6.4", "lib", "nunit.framework.dll"),
            src_dir.joinpath("packages", "NSubstitute.1.9.2.0", "lib", "net35", "NSubstitute.dll")
    ]
    ...
    src_dir.run("mono", "--runtime=3.5", "./packages/NUnit.Runners.2.6.4/tools/nunit-console.exe", test_src.output)
